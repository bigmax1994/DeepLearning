...
public struct Matrix<T: Codable> : CustomStringConvertible, ExpressibleByArrayLiteral, Codable {
    public var rows: Int
    public var cols: Int
    public var sparse: Bool = false
    public typealias MatrixType = T
    public var values: [T] = []
    
    public init(_ rows: Int, _ cols: Int, populating: (Int,Int) -> T  ) {
        self.rows = rows
        self.cols = cols
    ...
    public init(_ rows: Int, _ cols: Int, constant: T  ) {
        self.rows = rows
        self.cols = cols
    ...
    public init(withArray values: [T], rows: Int, cols: Int) {
        self.rows = rows
        self.cols = cols
        self.values = values
    }
    ...
    public func map<S>( _ mapper: (Int, Int, T) -> S ) -> Matrix<S> {
        var newValues : [S] = []
    ...
    public func map<S,U>(on:Matrix<U>, _ mapper: (T, U) -> S ) -> Matrix<S> {
    ...
    public func mapEach<S>(_ mapper: (T) -> S ) -> Matrix<S> {
    ...
    public func T() -> Matrix<T> {
        return self.transpose()
    }
    
    public mutating func appendRow( _ rowVector: Matrix<T> ) {
    ...
    public mutating func appendCol( _ colVector: Matrix<T> ) {
    ...
    public func rowAt(_ row: Int) -> Matrix<T> {
        let r : Int
        if row < 0 {
            r = self.rows + row
        } else {    
        ...
        let vals : [T] = Array(values[ r*self.cols ..< (self.cols*(row+1)) ])
        return Matrix<T>( [ vals ])
    }
    
    public func colAt(_ col: Int) -> Matrix<T> {
        let c : Int
        if col < 0 {
            c = self.cols + col
        } else {
...
extension Matrix where T : Comparable {
    public func findMax() -> (max: T?, row: Int, col: Int) {
        var max: T? = nil
        var maxAt: Int = 0
