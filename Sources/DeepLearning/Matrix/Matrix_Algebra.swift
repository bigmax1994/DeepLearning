...
extension Matrix where T : Semiring {
    ...
    public static func ●(a: Matrix<T>, b: Matrix<T>) -> Matrix<T> {
        return a.dot(b)
    }
    ...
    public static func I(_ size:Int) -> Matrix<T> {
    ...
    public static func *(a: Matrix<T>, b: T) -> Matrix<T> {
        return a.mapEach { $0 * b }
    }
    
    public static func *(a: T, b: Matrix<T>) -> Matrix<T> {
        return b.mapEach { $0 * a }
    }
    
    public static func zero(_ rows: Int, _ cols: Int)  -> Matrix<T>  {
